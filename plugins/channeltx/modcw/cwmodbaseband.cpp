...
//                                                                               //
// Morse Encoder (CW) modulator plugin — baseband implementation                //
//                                                                               //
...
bool CWModBaseband::handleMessage(const Message& cmd)
{
    ...
    {
        QMutexLocker mutexLocker(&m_mutex);
        const MsgConfigureCWModBaseband& cfg = static_cast<const MsgConfigureCWModBaseband&>(cmd);
    ...
    {
        const CWMod::MsgTXText& tx = static_cast<const CWMod::MsgTXText&>(cmd);
    ...
    else if (DSPSignalNotification::match(cmd))
    {
        ...
        qDebug() << "CWModBaseband::handleMessage: DSPSignalNotification basebandSampleRate:"
                 << notif.getSampleRate();
    ...
    {
        qDebug() << "CWModBaseband: unknown message";
        return false;
    }
}

void CWModBaseband::applySettings(
