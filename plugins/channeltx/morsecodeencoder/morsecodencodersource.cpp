...
//                                                                               //
// Morse Code Encoder — sample source implementation                             //
//                                                                               //
...
MorseCodeEncoderSource::MorseCodeEncoderSource() :
...
void MorseCodeEncoderSource::pull(SampleVector::iterator begin, unsigned int nbSamples)
{
    std::for_each(
        begin,
        begin + nbSamples,
...
void MorseCodeEncoderSource::pullOne(Sample& sample)
{
    ...
    // Check if the CW keyer has finished and load next queued message
    checkQueue();
...
void MorseCodeEncoderSource::checkQueue()
{
    QMutexLocker locker(&m_queueMutex);

    if (!m_txQueue.isEmpty() && m_cwKeyer.eom())
    {
        QString nextMsg = m_txQueue.dequeue();

        CWKeyerSettings cwSettings = m_cwKeyer.getSettings();
        cwSettings.m_text = nextMsg;
...
void MorseCodeEncoderSource::enqueueMessage(const QString& text)
{
    QMutexLocker locker(&m_queueMutex);
    m_txQueue.enqueue(text);
}

void MorseCodeEncoderSource::modulateSample(Real& modSample)
{
...
void MorseCodeEncoderSource::sampleToSpectrum(Complex sample)
{
    if (m_spectrumSink)
    {
        Complex out;
...
void MorseCodeEncoderSource::calculateLevel(Real& sample)
{
...
void MorseCodeEncoderSource::applySettings(const QStringList& settingsKeys, const MorseCodeEncoderSettings& settings, bool force)
{
...
void MorseCodeEncoderSource::applyChannelSettings(int channelSampleRate, int channelFrequencyOffset, bool force)
{
    qDebug() << "MorseCodeEncoderSource::applyChannelSettings:"
