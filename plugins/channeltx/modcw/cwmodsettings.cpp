...
void CWModSettings::resetToDefaults()
{
    ...
    m_gain = 0.0f;
    m_channelMute = false;
    m_text = "CQ CQ CQ DE SDRangel K";
    m_repeat = false;
    m_repeatCount = 1;
    m_rgbColor = QColor(100, 200, 100).rgb();
...
bool CWModSettings::deserialize(const QByteArray& data)
{
    ...
    if (d.getVersion() == 1)
    {
        ...
        d.readString(6, &m_text, "CQ CQ CQ DE SDRangel K");
        d.readBool(7, &m_repeat, false);
        d.readS32(8, &tmp, 1);
        m_repeatCount = tmp;

        d.readU32(20, &m_rgbColor, QColor(100, 200, 100).rgb());
