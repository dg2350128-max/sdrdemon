...
//                                                                               //
// CW Modulator — GUI implementation                                             //
//                                                                               //
...
void CWModGUI::on_rfBandwidth_valueChanged(int value)
{
    ui->rfBandwidthText->setText(QString::number(value));
    m_channelMarker.setBandwidth(value);
    m_settings.m_rfBandwidth = value;
    applySettings(QStringList("rfBandwidth"));
}

void CWModGUI::on_toneFrequency_valueChanged(int value)
{
    ui->toneFrequencyText->setText(QString::number(value));
...
{
    ui->gainText->setText(QString::number(value));
...
void CWModGUI::on_loop_toggled(bool checked)
{
    m_settings.m_loop = checked;
    applySettings(QStringList("loop"));
}

void CWModGUI::on_txButton_clicked()
{
    CWMod::MsgTXText *msg = CWMod::MsgTXText::create(m_settings.m_text);
    ...
    m_doApplySettings(true)
{
    ...
    m_channelMarker.setColor(QColor(255, 160, 32));
    m_channelMarker.setBandwidth(3000);
...
void CWModGUI::displaySettings()
{
    ...
    ui->rfBandwidth->setValue((int)m_settings.m_rfBandwidth);
    ui->rfBandwidthText->setText(QString::number((int)m_settings.m_rfBandwidth));
    ui->toneFrequency->setValue((int)m_settings.m_toneFrequency);
    ui->toneFrequencyText->setText(QString::number((int)m_settings.m_toneFrequency));
    ui->wpm->setValue(m_settings.m_wpm);
    ui->gain->setValue((int)m_settings.m_gain);
    ui->gainText->setText(QString::number((int)m_settings.m_gain));
    ui->channelMute->setChecked(m_settings.m_channelMute);
    ui->loop->setChecked(m_settings.m_loop);
...
void CWModGUI::makeUIConnections()
{
    ...
    QObject::connect(ui->rfBandwidth, &QSlider::valueChanged, this, &CWModGUI::on_rfBandwidth_valueChanged);
    QObject::connect(ui->toneFrequency, &QSlider::valueChanged, this, &CWModGUI::on_toneFrequency_valueChanged);
    QObject::connect(ui->wpm, &QSpinBox::valueChanged, this, &CWModGUI::on_wpm_valueChanged);
    QObject::connect(ui->gain, &QDial::valueChanged, this, &CWModGUI::on_gain_valueChanged);
    QObject::connect(ui->channelMute, &QToolButton::toggled, this, &CWModGUI::on_channelMute_toggled);
    QObject::connect(ui->loop, &QToolButton::toggled, this, &CWModGUI::on_loop_toggled);
