...
//                                                                               //
// Morse Encoder (CW) modulator plugin — settings implementation                //
//                                                                               //
...
void CWModSettings::resetToDefaults()
{
    ...
    m_keyerPort            = "";
    m_rgbColor             = QColor(0, 200, 0).rgb();
    m_title                = "Morse Encoder";
    m_streamIndex          = 0;
...
QByteArray CWModSettings::serialize() const
{
    ...
    s.writeBool(9, m_keyerEnabled);
    s.writeString(10, m_keyerPort);
...
bool CWModSettings::deserialize(const QByteArray& data)
{
    ...
    if (d.getVersion() == 1)
    {
        ...
        d.readReal(5, &m_wpm, 20.0f);
        d.readString(6, &m_text, "CQ CQ CQ DE SDRangel");
        d.readBool(7, &m_repeat, false);
        d.readS32(8,  &m_repeatCount, 1);
        d.readBool(9, &m_keyerEnabled, false);
        d.readString(10, &m_keyerPort, "");

        d.readU32(20, &m_rgbColor);
        d.readString(21, &m_title, "Morse Encoder");
...
void CWModSettings::applySettings(const QStringList& settingsKeys, const CWModSettings& settings)
{
    ...
    if (settingsKeys.contains("keyerEnabled")) {
        m_keyerEnabled = settings.m_keyerEnabled;
    }
    if (settingsKeys.contains("keyerPort")) {
        m_keyerPort = settings.m_keyerPort;
    }
...
QString CWModSettings::getDebugString(const QStringList& settingsKeys, bool force) const
{
    ...
    if (settingsKeys.contains("keyerEnabled") || force) {
        ostr << " m_keyerEnabled: " << m_keyerEnabled;
    }
    if (settingsKeys.contains("keyerPort") || force) {
        ostr << " m_keyerPort: " << m_keyerPort.toStdString();
    }
