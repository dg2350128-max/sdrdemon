...
//                                                                               //
// Morse Encoder (CW) modulator plugin — main channel class implementation      //
//                                                                               //
...
MESSAGE_CLASS_DEFINITION(CWMod::MsgReportTx, Message)
...
// ---------------------------------------------------------------------------
// ChannelAPI / BasebandSampleSource
// ---------------------------------------------------------------------------
...
void CWMod::setCenterFrequency(qint64 frequency)
{
    ...
    if (m_guiMessageQueue) {
        MsgConfigureCWMod* msgToGUI = MsgConfigureCWMod::create(
...
bool CWMod::handleMessage(const Message& cmd)
{
    ...
    {
        const MsgConfigureCWMod& cfg = static_cast<const MsgConfigureCWMod&>(cmd);
    ...
    {
        MsgTx* msg = new MsgTx(static_cast<const MsgTx&>(cmd));
        m_basebandSource->getInputMessageQueue()->push(msg);
        return true;
    }
    else if (MsgTXText::match(cmd))
    {
        MsgTXText* msg = new MsgTXText(static_cast<const MsgTXText&>(cmd));
        m_basebandSource->getInputMessageQueue()->push(msg);
        return true;
    }
    else if (DSPSignalNotification::match(cmd))
    {
        const DSPSignalNotification& notif = static_cast<const DSPSignalNotification&>(cmd);
    ...
    bool force)
{
    ...
    CWModBaseband::MsgConfigureCWModBaseband* msg =
        CWModBaseband::MsgConfigureCWModBaseband::create(settingsKeys, settings, force);
    m_basebandSource->getInputMessageQueue()->push(msg);
    ...
    QString& errorMessage)
{
    (void)errorMessage;
    (void)response;
    // Full Swagger integration would populate response.setCwModSettings(...)
    // For now return OK without body population
    return 200;
}
...
void CWMod::webapiReverseSendSettings(
    ...
    bool force)
{
    ...
    (void)settings;
    (void)force;
    // Reverse API support would be implemented here
}
