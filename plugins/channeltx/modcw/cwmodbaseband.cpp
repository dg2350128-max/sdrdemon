...
//                                                                               //
// CW Modulator — baseband implementation                                        //
//                                                                               //
...
bool CWModBaseband::handleMessage(const Message& cmd)
{
    ...
    else if (CWMod::MsgTXText::match(cmd))
    {
        ...
        CWKeyerSettings cwSettings = m_source.getCWKeyer().getSettings();
        cwSettings.m_text = tx.m_text;
        cwSettings.m_mode = CWKeyerSettings::CWText;
        m_source.getCWKeyer().getInputMessageQueue()->push(CWKeyer::MsgConfigureCWKeyer::create(cwSettings, false));
        m_source.getCWKeyer().resetText();
        return true;
    }
